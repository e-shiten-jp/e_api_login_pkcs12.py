...
FNAME_URL_INFO = "./.auth/file_url_info.txt"                # 本番・デモの切り替えをディレクトリのシンボリックリンクで行う場合。
...
def func_replace_sendin_url(str_url, str_auth_id):
    # 変数で保持しているsAuthIdの値
    auth_id = "TWpBeE1EVTFNakUzTVRjd05pMHhNVFl0TmpZME9DMHdNVE09"

    # 1. 一部を非表示にした文字列を作成（前後4文字を残して間を * にする例）
    masked_id = auth_id[:4] + "*" * (len(auth_id) - 8) + auth_id[-4:]

    # 2. replaceを使って元の文字列内にある値を置き換える
    output_str = url_str.replace(auth_id, masked_id)
...
def func_print_sendding_url(str_sending_url, str_auth_id):
    # 1. 一部を非表示にした文字列を作成（前後3文字を残して間を * にする例）
    masked_id = str_auth_id[:3] + "*" * (len(str_auth_id) - 6) + str_auth_id[-3:]

    # 2. replaceを使って元の文字列内にある値を置き換える
    output_str = str_sending_url.replace(str_auth_id, masked_id)

    print('--- 送信電文 -------------------------------------------')
    print(output_str)


def func_print_response(str_output):
    print('--- 受信電文 -------------------------------------------')
    print(str_output)
    print('--------------------------------------------------------')
...
def func_login_auth(int_p_no, str_sAuthId, dic_url_info):
    ...
    # 送信電文の出力
    func_print_sendding_url(str_url, str_sAuthId)
    ...
    # 受信電文の出力
    func_print_response(str_api_response)
