...
def func_print_auth_url(str_sending_url, str_auth_id):
    # 1. AuthIDの一部を非表示にした文字列を作成（前後3文字を残して間を * にする例）
    masked_id = str_auth_id[:3] + "*" * (len(str_auth_id) - 6) + str_auth_id[-3:]

    # 2. replaceを使って元の文字列内にあるAuthID値を置き換える
    str_output = str_sending_url.replace(str_auth_id, masked_id)
...
def func_login_auth(int_p_no, str_sAuthId, dic_url_info):
    ...
    # login認証時の送信電文の一部を隠し文字にして出力
    func_print_auth_url(str_url, str_sAuthId)
